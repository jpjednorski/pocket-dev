...
echo ""
echo -e "${BLUE}${BOLD}╔════════════════════════════════════════╗${NC}"
echo -e "${BLUE}${BOLD}║          📱 pocket-dev installer       ║${NC}"
echo -e "${BLUE}${BOLD}╚════════════════════════════════════════╝${NC}"
echo ""
...
if [[ -n "$SCRIPT_DIR" && -f "$SCRIPT_DIR/justfile" ]]; then
    ...
    echo -e "${BLUE}▸${NC} Running via curl/wget"
    ...
    # Clone repo
    echo -e "${BLUE}▸${NC} Cloning pocket-dev..."
    if [[ -d "$INSTALL_DIR" ]]; then
        echo -e "${YELLOW}▸${NC} $INSTALL_DIR exists, updating..."
        cd "$INSTALL_DIR"
...
if [[ -n "${TAILSCALE_AUTH_KEY:-}" && -n "${NTFY_TOPIC:-}" ]]; then
    ...
    echo -e "${BLUE}▸${NC} Creating .env from provided env vars..."
    cat > .env <<EOF
...
if [[ -z "${TAILSCALE_AUTH_KEY:-}" ]]; then
    echo -e "${RED}✗${NC} TAILSCALE_AUTH_KEY not set in .env"
    echo "  Get one from: https://login.tailscale.com/admin/settings/keys"
    exit 1
fi

if [[ -z "${NTFY_TOPIC:-}" ]]; then
    echo -e "${RED}✗${NC} NTFY_TOPIC not set in .env"
